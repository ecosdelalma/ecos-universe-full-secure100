...
export default function ConstellationGraph(){ const r=useRouter(); const width=800,height=480;
  return (<div className="relative rounded-2xl overflow-hidden ecos-glow">
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto block">
      <defs><radialGradient id="bg" cx="50%" cy="30%" r="70%"><stop offset="0%" stopColor="rgba(255,255,255,0.08)"/><stop offset="100%" stopColor="rgba(0,0,0,0.6)"/></radialGradient></defs>
      ...
      {NODES.map(n=>(<g key={n.id} transform={`translate(${(n.x/100)*width}, ${(n.y/100)*height})`} onClick={()=>r.push(n.route)} style={{cursor:'pointer'}}>
